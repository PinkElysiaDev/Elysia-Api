...
> 以前 AI 助手只会帮你配协议；现在你说「帮我建个叫 test 的模型组，把最新的 grok 和 gpt 放进去，再配个只给这个组用的 Key」，它就真的去建——中途每个写操作都会先等你点头，执行到哪一步、跑了什么命令，全都看得见。人在外面，用支持 MCP 的客户端连回家里这台机器，还能接着指挥它干活。

### 全新的通用智能体（a305fea / 7788f37 / ecd7fb2 / a727b98）

- **什么都能聊着办**：内置通用网关助手（a305fea），覆盖协议接入、模型源/模型组/API Key 管理、用量统计分析、失败请求下钻、系统日志查询。
- **elysia 命令行**：模型侧工具收敛为一条 `elysia` CLI（33 条命令、九大能力域），配 bash 工具与内容感知门控探针（ecd7fb2 / 17f9728 / 7788f37）——就像给助手一个终端。
- **提示词体系深度重构**：能力优先、渐进披露（a727b98），并修掉了「自称没有工具」的拒绝问题。
- **会问、会规划、会省着说**：能暂停向你提问或请确认方案（bb8bb98）；计划模式先出方案、确认后执行；长对话自动摘要压缩上下文（d134314）；密钥在所有出口脱敏、安全工具并行执行（e82134a）。
- **过程全透明**：工具执行显示为状态行（18e2536），CLI 批内逐条命令实时报进度（31203e3）；会话标题由任务自动命名（1256d9b）；模型组/源/Key 都能改名（ad6f516）。

### 远程指挥：REST / MCP / A2A（0182c06 / 9e2b233 / de66326 / c14d74f）

- 新增**专用的远程访问 Key**（AI 助手作用域，与网关调用 Key 完全分离，管理页可独立启停）。
- 暴露三种远程面：管理 REST、MCP 端点（九个工具，标准 MCP 客户端直接连）、A2A 端点（双线制、审批可续）。Koishi 插件等外部程序已在用。
- 远程发生的对话、审批、提问会同步出现在网页端（23a4451）；远程新建会话自动继承你最近的模型与设置（6bf214f）。

### 对话体验精修

- **Enter 直接发送**（95158e1）+ 审批/提问卡 Esc 快捷拒绝或跳过（8d34998），快捷键集中在 AI 助手首页的设置窗口（46c53b1）。
- 上传图片有缩略图、点击放大灯箱（098dfa1）；停止按钮真的能停且状态落库（df7096a）；启动时自动打开控制台（a342660）；窄窗口下侧栏不再挤压对话区、开合动画流畅（3af0b07 / fec08f1）；思考档位新增「极高」（d9434b8）。

### 预置协议 v2：与内置线完全等价（2831ecd / e781717 / 15c955a）

- 四份预置协议（chat-completions-api / responses-api / anthropic-api / gemini-api）补齐思考整形、思考签名跨轮回传、拒答与引用注解、缓存用量、模型发现配置等全部高级能力；协议 ID 去厂商化改名（ed5d6a8），并有 parity 金样测试保证与内置实现逐项等价（f7b63c9）。
- **未被你改动过的预置启动时自动升级到 v2**，改过的原样保留。

### 安全与运维

- SSRF 防护改为**可配置的出站禁止 IP 段列表**（41035b2）：上游在本机/内网被拦时，运行配置页删掉对应段即可放行，AI 助手也能代办（update_outbound_policy 工具）。
- 深度审计修复 24 项引擎/调用方/前端/存储缺陷（de953f1 等），含批量审批绕权、探针绕过与脱敏泄露（69aa1d0）；`/health` 上报版本（f4af965）；修复 `__shutdown` 后进程驻留（8c16a0a）；npm 平台二进制包发布链落地并统一命名（04449e5 / 936375a）。

### 两轮代码质量治理（行为保持重构）

约 30 个纯重构提交（dd21e80 / 08938e7 / a505b67 / 6982379 / 1082cf1 等）：超长转换函数拆分、重复逻辑合并、死代码清理、字面量常量化、大文件按域拆分——全部以「测试全绿」为门禁，用户可见行为不变。

### 致谢

本版本的全部 160 个提交由 [@PinkElysiaDev](https://github.com/PinkElysiaDev) 完成。同时感谢项目的所有贡献者——[@vioaki](https://github.com/vioaki)（登录页与 macOS 体验设计）、[@LingLambda](https://github.com/LingLambda)、[@Procyon-Nan](https://github.com/Procyon-Nan)——你们的贡献仍在每一个版本里服役！

### 升级说明

老数据零手工操作：预置协议自动升 v2、旧协议 ID 自动改名（引用同步重写）、出站旧配置自动迁移、npm 平台包更名为 `elysia-api-<os>-<arch>`。完整技术变更见仓库 [CHANGELOG.md](https://github.com/PinkElysiaDev/Elysia-Api/blob/deploy/CHANGELOG.md)。

<!-- release-details -->

自 v1.4.0 以来合入 deploy 的完整技术变更（按主题归纳）：

### 收尾批次：快捷键、改名与界面修复

- **agent 补齐模型组与 API Key 改名**：`elysia group update --name <新组名>`（组名即客户端调用模型名，改名影响与 Key 授权不迁移的提示已写入帮助）与
  `elysia key update --new-name <新名>`（目标名占用报错，可与启停/换密钥同批）；模型源改名此前已支持。
...
- **agent 引擎**：工具名跨包常量（agent.ToolName*）；上下文压缩阈值上提 const；
  ...
  轮次错误按 404/500 分级；截断方向类型化（ClampHead/ClampTail）；
  PlanStale→PlanStaleRounds；ParseAsk 从包级全局移入 Options 构造期注入；
  agent.go 拆出 tool_context.go / masking.go；工具错误码统一为稳定
  snake_case（动态错误文本只留 Summary）。
- **agent 工具层**：draftProtocol/resolveTestTarget 收敛四处重复前置；
  用量查询参数一次解码（原先同一段 JSON 解三次）；限额/天数/偏移常量化
  并共用 clampInt；update_group 的字段补丁与草稿离线自检各自成函数。
- **转换引擎**：八个百行级转换函数各自拆出分支辅助件（消息分类/工具调用
  渲染/part 转换/单项转换/校验三段/根组装三段）；思考整形双实现合一
  （applyAnthropicThinking / buildGeminiThinkingConfig / buildResponsesReasoning
  同时服务内置线与自定义协议模板）；占位符扫描三合一（strict/lenient 保语义）；
  responseDirectFields 的 legacy 列被 resolveCustomMapping 真正使用
  （五个 mappings 简写键从静默忽略变为按表生效，无预置受影响）；
  responses 渲染器 text/refusal 四胞胎参数化为统一槽位；Claude/Gemini
  取回分支独立成函数。
- **转发管线**：六个 handler 各自复制的失败闭包统一为 relayFailWriter；
  ...
  校验文案与允许集对齐（顺修文案漂移）。
- **storage**：removeGroupFromTokens 复用 updateTokenGroupsTx 骨架（清空
  token 名单由骨架返回，禁用策略留在删除路径）；幂等 ALTER 链清单化；
  回填拆为 ready 对账 / 分块回填两个相位。
- **前端**：删零引用的类型字段与导出、z-index 登记表收缩、不可达 CSS 变体
  与空三元/孤儿注释清除；守卫与命名收敛、时限与阈值常量化；agent 四个
  大文件拆为十一 个聚焦模块（markdown/approval-cards/live-view/
  use-chat-scroll/composer-dock/context-gauge/use-agent-drafts/
  workspace-header/plan-view/draft-view/activity-view）；FieldSelect 与
  useDismissable 跨文件合一（模型选择器补上 Escape 关闭）；工具行复用全局
  CopyButton；runtime-config 内联异步闭包提为具名 handler；流式状态
  role=status、错误横幅 role=alert、图标按钮补 aria-label、您/你统一。

### 缺陷修复（总览统计 / 本地草稿 / 交互一致性）

- **会话统计修复**：非 assistant 消息的 usage 落库为空串，json_extract 遇空串
  抛错并中止聚合且被吞掉——轮数与 token 恒为 0、已结束会话被标成空对话。
  现按空串短路求和并检查迭代错误；两个标题写入路径统一钳制 64 字。
- **草稿归属修复**：切换或新建会话时上一会话的未发送内容会播种进新会话
  输入框并随击键写入新会话草稿；重进同一会话会回退到首次进入时的旧快照。
  草稿现在带会话归属，ChatPanel 等草稿读毕再挂载，每次进入视图重读。
- **草稿存储加固**：IndexedDB 连接改为复用（原先每次操作新开且不关），
  回写加 300ms 防抖（附件可达数 MB），发送时取消挂起并立即清缓存；
  会话详情慢响应不再覆盖已切换的会话。
- **交互一致性**：方案卡裸 Enter 不再隐式提交（仅 Ctrl/Cmd+Enter，与主输入
  框一致）；轮数条非活动数字对比度提到 AA；工具行补「更新会话标题」中文名。

### AI 助手总览与协议列表

- 总览标题去掉说明行；「AI」改用正文字体、中文保持 display 字体且不再加粗。
- 会话标题由模型在执行中用 update_title 改写成不超过 16 字的任务概括
  （首条消息截断仍作开口前的占位）。
- 卡片副文案改为状态与统计：待确认（审批/提问/方案确认）/ 进行中 / 有未发送
  内容 / 已结束 / 空对话，加轮数与累计 token。列表接口附带聚合，不读消息正文。
- 未发送的文本与附件按会话缓存在本机 IndexedDB：离开再回来仍在，发送成功
  或删除会话后清除。
- 会话页再做减法：助手消息去掉头像列，轮数条去掉圆形描边。
- 方案确认卡：「确认执行」改为「执行方案」，移除「需要修改」；修改意见
  从输入框右侧的发送钮或 Ctrl/Cmd+Enter 发出，空内容不发送。
- 协议设计器：数据行去掉与表头叠加的实线底边（第一条下不再是重线）；
  预置协议置顶，组内仍按 id 排序。

### AI 助手界面整理

- 总览页改用与「模型组/模型源」一致的 PageHeader（display 字体大标题），
  会话卡片网格去掉 max-w 限宽占满页面，并消除与页面容器的双重缩进。
- 会话工作区（聊天列、轮数条、侧栏视图）隐藏滚动条，滚动能力不变。
- 审批/提问/方案确认卡移入底部输入区——待确认时输入框整块让位
  （Zcode 式：交互发生在输入位置），消息流不再插卡。
- 对话界面做减法：去掉 assistant 消息里与工具结果行重复的工具 chips、
  工具行下的「在动态页查看」按钮与顶栏双行副标题，消息间距收紧。
