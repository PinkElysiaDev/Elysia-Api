...
本文件记录 Elysia-API 每个版本的完整发布说明，新版本在前。发版时
`.github/workflows/release.yml` 会自动提取 tag 对应版本的小节作为 GitHub
Release 正文——**发版前必须先在此写下该版本的完整说明**，否则发布流程会
失败（这是有意为之，避免发布空说明）。
