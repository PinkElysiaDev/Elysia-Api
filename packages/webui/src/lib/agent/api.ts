...
import type { AgentSession, AgentSessionDetail, AgentSettings } from "./types";
...
export async function updateAgentSession(
  ...
  return request<AgentSession>(`/agent/sessions/${id}`, {
    method: "PATCH",
    body: input,
  });
}
...
export async function restoreAgentDraft(id: string): Promise<AgentSession> {
  return request<AgentSession>(`/agent/sessions/${id}/restore-draft`, {
    method: "POST",
  });
}
...
export async function clearAgentMessages(
  ...
  await request(`/agent/sessions/${id}/messages`, {
    method: "DELETE",
    query: { afterSeq },
  });
}
...
export async function stopAgentTurn(id: string): Promise<boolean> {
  const data = await request<{ stopped: boolean }>(
