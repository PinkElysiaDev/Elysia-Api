...
// 连接只开一次并复用：草稿随击键高频写入，每次 open 新连接且不关会泄漏。
...
export async function loadComposerDraft(
  sessionId: string,
): Promise<AgentComposerDraft | null> {
  try {
    const db = await getDb();
    return await new Promise((resolve) => {
      const request = db.transaction(STORE).objectStore(STORE).get(sessionId);
      request.onsuccess = () =>
        resolve((request.result as AgentComposerDraft | undefined) ?? null);
...
export async function sessionsWithDrafts(
  sessionIds: string[],
): Promise<Set<string>> {
  ...
  try {
    ...
    await new Promise<void>((resolve) => {
      ...
      request.onsuccess = () => {
        ...
        if (
          sessionIds.includes(String(cursor.key)) &&
          (value.text.trim() || value.documents.length > 0)
        ) {
