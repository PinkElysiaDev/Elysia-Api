...
export function SessionOverview({
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-6 pb-8 pt-4">
          ...
          >
            <div className="flex items-start gap-2 pr-7">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 truncate text-2xs text-muted-foreground">
                  {session.mode === 'edit' ? `编辑协议 ${session.protocolId ?? ''}` : '网关运维 · 协议接入 · 统计分析'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2 text-2xs text-muted-foreground">
              {session.plan && session.plan.length > 0 ? (
                <span className="tnum">
                  方案 {session.plan.filter((step) => step.status === 'done').length}/{session.plan.length}
                </span>
              ) : null}
              {session.settings.planMode ? <span className="text-amber">计划模式</span> : null}
      ...
      {sessions.length === 0 ? (
        <div className="mx-auto mt-10 flex max-w-5xl items-center justify-center gap-2 text-2xs text-muted-foreground">
