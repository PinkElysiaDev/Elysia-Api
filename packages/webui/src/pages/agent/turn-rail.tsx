...
export function TurnRail({
  ...
  return (
    ...
    >
      {turns.map((turn, index) => {
        ...
        return (
          ...
          >
            <span
              className={cn(
                'tnum flex h-[18px] w-[18px] items-center justify-center rounded-full border text-[10px] leading-none transition-colors',
                active ? 'border-rose font-medium text-rose' : 'border-border text-muted-foreground',
              )}
            >
              {index + 1}
            </span>
      ...
      {live.running ? <span className="dot dot-ok mt-1 shrink-0" aria-label="本轮进行中" /> : null}
    </nav>
  )
}
