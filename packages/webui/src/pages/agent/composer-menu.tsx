...
export function PermissionMenu({
  ...
  const levelLabel =
    PERMISSION_LEVELS.find((item) => item.value === level)?.label ??
    "变更前确认";
      ...
      icon={
        <Shield
          className={cn(
            ...
            planMode ? "text-amber" : "text-muted-foreground",
          )}
        />
      }
      label={
        planMode ? <span className="text-amber">计划模式</span> : levelLabel
      }
...
function effortLabel(effort: AgentThinkingEffort | undefined): string {
  return (
    THINKING_EFFORTS.find((item) => item.value === (effort ?? ""))?.label ??
    "思考"
  );
}
    ...
    <MenuShell
      icon={
        <Gauge
          className={cn(
            "h-3.5 w-3.5 shrink-0",
            enabled ? "text-amber" : "text-muted-foreground",
          )}
        />
      }
