...
function toRows(spec: AgentChartSpec) {
  ...
  for (let i = 0; i < length; i += 1) {
    ...
    ;(spec.series ?? []).forEach((series, seriesIndex) => {
      row[series.name || `系列${seriesIndex + 1}`] = Number(series.data?.[i] ?? 0)
    })
    rows.push(row)
  }
  return rows
...
export function ChartBlock({ spec }: { spec: AgentChartSpec }) {
  ...
  const seriesNames = (spec.series ?? []).map((series, index) => series.name || `系列${index + 1}`)
  const height = 240
        ...
        <ResponsiveContainer width="100%" height={height}>
          {spec.type === 'pie' ? (
            <PieChart>
              ...
              />
              <Pie
                data={rows.map((row) => ({ name: String(row.name), value: Number(row[seriesNames[0]] ?? 0) }))}
                dataKey="value"
                nameKey="name"
                innerRadius="45%"
                outerRadius="80%"
            ...
            <LineChart data={rows} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
              ...
              <XAxis dataKey="name" tick={CHART_TICK} interval="preserveStartEnd" />
              ...
              {seriesNames.map((name, index) => (
                <Line key={name} dataKey={name} type="monotone" stroke={modelColor(index)} strokeWidth={2} dot={false} />
              ))}
            </LineChart>
          ) : (
            <BarChart data={rows} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
              ...
              <Legend wrapperStyle={{ fontSize: 11 }} />
              {seriesNames.map((name, index) => (
                <Bar key={name} dataKey={name} fill={modelColor(index)} fillOpacity={0.75} radius={[3, 3, 0, 0]} maxBarSize={28} />
              ))}
            </BarChart>
          )}
        </ResponsiveContainer>
