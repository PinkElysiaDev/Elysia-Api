  ...
  const flat = useMemo(
    () =>
      groups.flatMap((group) =>
        group.models.map((model) => ({ source: group.source, model })),
      ),
    [groups],
  );
  ...
  return (
    <div ref={rootRef} className="relative" onKeyDown={onKeyDown}>
      <button
        ...
        title={
          selected
            ? `${selected.sourceName ?? sourceId} / ${modelName}`
            : "按模型源分组选择模型"
        }
      >
        ...
        {selected?.visionCapable ? (
          <Eye className="h-3 w-3 shrink-0 text-jade" />
        ) : null}
      ...
      {open ? (
        ...
        >
          ...
          <div
            ...
            role="listbox"
            aria-label="模型列表"
            className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-1 pt-1.5"
          >
            {flat.length === 0 ? (
              <p
                role="status"
                ...
                <div key={group.source.id} className="mb-1">
                  <p className="flex items-baseline gap-1.5 px-2 pb-0.5 pt-1.5 text-2xs text-muted-foreground">
                    <span className="font-medium">
                      {group.source.name || group.source.id}
                    </span>
