...
import { CHART_ENTER_MS, evenIndexTicks, niceTicks, useCommittedRange, useEnterAnimation } from '@/components/usage-chart-model'
import { Seg } from '@/components/ui/seg'
import { ErrorState } from '@/components/ui/states'
import { useUsagePulse } from '@/lib/hooks'
import { cn, CHART_TICK, compactNumber, formatNumber } from '@/lib/utils'
import { formatHm, PULSE_OPTIONS, pulseWindow, type PulseRange } from './overview-time'
...
export function PulseSection({ minuteTick }: { minuteTick: number }) {
  ...
  const series = useMemo(() => {
    ...
    const out: { t: number; label: string; rpm: number; avgMs: number; p95Ms: number; requests: number; tokens: number; tpm: number }[] = []
    while (t < toMs) {
      const p = byT.get(t)
  ...
  const xTicks = useMemo(() => evenIndexTicks(series, 13, (d) => d.t), [series])
        ...
        <div className="flex flex-col gap-9 border-b lg:border-b-0 lg:border-r border-border/40 pb-4 lg:pb-[26px] pr-0 lg:pr-6">
          <div>
            <div className="flex items-baseline gap-1.5">
              <span className="tnum font-display text-2xl font-medium leading-none tracking-tight text-foreground sm:text-[34px]">
                {shown.rpm.toFixed(1)}
              </span>
            ...
            <ChartFrame key={chartRange} height={SINGLE_PULSE_CHART.height}>
              ...
              >
                ...
                <XAxis
                  ...
                  ticks={xTicks}
                  interval={0}
                  tickFormatter={(v: number) => formatHm(Number(v))}
                  tick={CHART_TICK}
                  tickLine={false}
